...
class MentalHealthChatbot:
    ...
    def respond(self, user_input):
        ...
        if main_issue:
            response = self.generate_solution(main_issue)
            self.say(response)
            return response
        else:
            main_feeling = self.extract_main_feeling(user_input)
            if main_feeling:
                response = self.generate_solution(main_feeling)
                self.say(response)
                return response
            else:
                response = "I'm sorry, I couldn't understand the issue. Could you please rephrase?"
                self.say(response)
                return response
    ...
    def generate_solution(self, issue_or_feeling):
        solutions = {
            ...
            "motivation": "Remember the childhood dreams you had? Don't you think we need to accomplish that?"
        }
